...
dir = "/cs/usr/tomer.cohen13/lab/NN/NanoNetPDBs"
sub_dirs = [str(i) for i in range(1,12)]
...
if __name__ == '__main__':
    ...
    os.chdir(dir)
    maxi = 0
    for dir_num in sub_dirs:
        os.chdir(dir_num)
        for pdb_dir in os.listdir(os.getcwd()):
            os.chdir(pdb_dir)
            model = PDBParser().get_structure("ref.pdb", "ref.pdb")[0]["H"]
            seq, aa_residues = get_seq(model)
            s, e = cdr2(seq)
            if (e-s) > maxi:
                maxi = e-s
                # print("{}, {} : {}  ({}-{})".format(dir_num, pdb_dir, e-s, s,e))
            os.chdir("..")
        os.chdir("..")
    print(maxi)
