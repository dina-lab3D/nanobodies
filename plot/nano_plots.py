...
def plot_points(pdb_path, pdb_name, score_name):
    df = pd.read_csv(pdb_path + "/" + pdb_name + "/scores.txt", sep=" ", header=None, names=COL)
...
if __name__ == '__main__':
    ...
    for file in os.listdir(sys.argv[1]):
        if os.path.isdir(sys.argv[1] + "/" + file) and os.path.exists(sys.argv[1] + "/" + file + "/scores.txt"):
            plot_points(sys.argv[1], file, "soap_score")
            i += 1
            if i == 10:
                break
